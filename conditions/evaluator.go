...
func Evaluate(ctx context.Context, expr Expr, args map[string]interface{}) (bool, error) {
	if expr == nil {
		return false, errors.New("expression is nil")
	}

	result, err := evaluateSubtree(ctx, expr, args)
	if err != nil {
		return false, err
	}
	...
	return false, fmt.Errorf("Evaluate(%s): %v", expr.String(), result)
}

func deriveZeroType(otherOperand Expr) Expr {
	if otherOperand == nil {
		// Nothing we can do
		return nil
	}
...
func evaluateBinary(ctx context.Context, expr Expr, args map[string]interface{}) (Expr, error) {
	switch expr.(type) {
	case *BinaryExpr:
		// We deal with this below.

	default:
		return falseExpr, fmt.Errorf("evaluateBinary(): '%s' is not a binary expression", expr.String())
	}

	binaryExpression := expr.(*BinaryExpr)
	// Get the LHS and RHS
	lv, lhsErr := evaluateSubtree(ctx, binaryExpression.LHS, args)
	rv, rhsErr := evaluateSubtree(ctx, binaryExpression.RHS, args)
	if lhsErr == nil && rhsErr == nil {
		// Happy-path case, no errors on RHS or LHS
		return applyOperator(binaryExpression.Op, lv, rv)
	}

	if lhsErr != nil {
		if !viper.GetBool("config.undefined_is_zero") {
			// We are not to give unset {variables} their default value, as per config
			return falseExpr, lhsErr
		}

		// There is an error with the LHS
		// So we have a LHS but we have no RHS
		if rhsErr != nil {
			// If we got an error on both LHS and RHS, then there is nothing we can do
			return falseExpr, lhsErr
		}

		if !strings.Contains(lhsErr.Error(), "not found") {
			// Whatever the problem is, it is not a "not found" error
			return falseExpr, lhsErr
		}

		// We got an error on LHS only, AND that error is because of a variable not being set.
		// Infer the zero type for it and use that value for the LHS
		if lv = deriveZeroType(rv); lv == nil {
			// We could not figure out the zero value for the LHS
			return falseExpr, lhsErr
		}

		if viper.GetBool("config.debug") {
			log.Printf("evaluateBinary(%s): replacing with zero-value %s", binaryExpression.LHS, lv)
		}
		return applyOperator(binaryExpression.Op, lv, rv)
	}
	...
	if !viper.GetBool("config.undefined_is_zero") {
		// We are not to give unset {variables} their default value, as per config
		return falseExpr, rhsErr
	}

	if !strings.Contains(rhsErr.Error(), "not found") {
		return falseExpr, rhsErr
	}

	// We got an error on RHS only, AND that error is because of a variable not being set.
	// Infer the zero type for it and use that value for the RHS
	if rv = deriveZeroType(lv); rv == nil {
		// We could not figure out the zero value for the RHS
		return falseExpr, rhsErr
	}

	if viper.GetBool("config.debug") {
		log.Printf("evaluateBinary(%s): replacing with zero-value %s", binaryExpression.RHS, rv)
	}
	return applyOperator(binaryExpression.Op, lv, rv)
}

// evaluateSubtree performs given expr evaluation recursively
func evaluateSubtree(ctx context.Context, expr Expr, args map[string]interface{}) (Expr, error) {
	if expr == nil {
	...
	switch n := expr.(type) {
	case *ParenExpr:
		return evaluateSubtree(ctx, n.Expr, args)
	case *BinaryExpr:
		return evaluateBinary(ctx, expr, args)
	case *VarRef:
		index := n.Val
		var value any
		if err != nil {
		...
		if _, ok := args[index]; !ok {
			// This is a variable lookup, and the variable is not found.
			// Check the context
			if value = ctx.Value(index); value == nil {
				// The variable is not found in the context either
				return falseExpr, fmt.Errorf("argument: %v not found", index)
			}
		} else {
			// The variable is found in the args
			value = args[index]
		}
		...
		switch kind {
		case reflect.Int:
			return &NumberLiteral{Val: float64(value.(int))}, nil
		case reflect.Int32:
			return &NumberLiteral{Val: float64(value.(int32))}, nil
		case reflect.Int64:
			return &NumberLiteral{Val: float64(value.(int64))}, nil
		case reflect.Float32:
			return &NumberLiteral{Val: float64(value.(float32))}, nil
		case reflect.Float64:
			return &NumberLiteral{Val: float64(value.(float64))}, nil
		case reflect.String:
			if num, ok := value.(json.Number); ok {
				f, err := num.Float64()
				if err != nil {
					return falseExpr, fmt.Errorf("Unsupported JSON Number %v type: %s", value, kind)
				}
				return &NumberLiteral{Val: f}, nil
			}
			return &StringLiteral{Val: value.(string)}, nil
		case reflect.Bool:
			return &BooleanLiteral{Val: value.(bool)}, nil
		case reflect.Slice:
			switch value.(type) {
			case []string:
				ssl := NewSliceStringLiteral(value.([]string))
				return ssl, nil
			case []int:
			...
			case []json.Number:
				snl := &SliceNumberLiteral{}
				for _, v := range value.([]json.Number) {
					f, _ := v.Float64()
