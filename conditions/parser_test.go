...
func TestValid(t *testing.T) {
	...
	for _, td := range validTestData {
		...
		r, err = Evaluate(context.TODO(), expr, td.args)
		if err != nil {
			if td.isErr {
				continue
			}
...
func TestSetDefaultEpsilon(t *testing.T) {
	...
	t.Run("0.1 == 0.1", func(t *testing.T) {
		...
		expr, _ := p.Parse()
		r, err := Evaluate(context.TODO(), expr, map[string]interface{}{"foo": 0.1})
		assert.True(t, r)
	...
	t.Run("0.1 == 0.100000000001", func(t *testing.T) {
		...
		p := NewParser(strings.NewReader("{foo} == 0.1"))
		expr, _ := p.Parse()
		r, err := Evaluate(context.TODO(), expr, map[string]interface{}{"foo": 0.100000000001})
		assert.True(t, r)
		assert.NoError(t, err)
	})
	...
	t.Run("0.1 != 0.100001", func(t *testing.T) {
		...
		p := NewParser(strings.NewReader("{foo} == 0.1"))
		expr, _ := p.Parse()
		r, err := Evaluate(context.TODO(), expr, map[string]interface{}{"foo": 0.100001})
		assert.False(t, r)
		assert.NoError(t, err)
	})
...
func TestReadmeExample(t *testing.T) {
	...
	r, err := Evaluate(context.TODO(), expr, data)
	if err != nil {
		t.Error(err)
	}
...
func TestJSON(t *testing.T) {
	...
	for _, test := range tests {
		...
		_ = json.Unmarshal([]byte(test.jsonStr), &data)
		r, err := Evaluate(context.TODO(), expr, data)
...
func BenchmarkParser(b *testing.B) {
	...
	for n := 0; n < b.N; n++ {
		_, _ = Evaluate(context.TODO(), expr, args)
	}
}
