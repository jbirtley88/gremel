...
func CreateDBFromFile(ctx data.GremelContext, database db.GremelDB, tableName string, datafile string) error {
	...
	case ".csv":
		// Parse CSV file
		return fmt.Errorf("CreateDB(%s): TODO(): unimplemented file type: %s", datafile, ext)
...
func CreateDBFromReader(ctx data.GremelContext, database db.GremelDB, tableName string, input io.Reader, parser data.Parser) error {
	rows, headings, err := parser.Parse(input)
	if err != nil {
		return fmt.Errorf("CreateDBFromReader(%s): failed to parse data: %w", tableName, err)
	}
	ctx.Values().SetValue(tableName+".headings", headings)
	...
	db := db.NewGremelDB(tableName)
	err = db.CreateSchema(tableName, rows[0])
	if err != nil {
		return fmt.Errorf("CreateDBFromReader(%s): failed to create schema: %w", tableName, err)
	}

	// Insert all the rows
	for rowNum, rowMap := range rows {
		// Ensure all rows have the same columns as the first row
		err = db.InsertRows(tableName, data.NewSingleRow(rowMap))
		if err != nil {
